...
As stated, out of the box the MEK6800D2 has one serial interface driven by an 6850 ACIA. The serial bitstream is converted to tones of 1200 and 2400 Hz to be fed into a cassette recorder and the reverse for reading data from a cassette recorde. The JBUG program can store ('punch') and load programs on/from tape using the so called _Kansas City Standard_ that sends/receives data at 300 bps using these two frequencies, 8 cycles of a 2400 Hz tone for a '1' and 4 cycles of a 1200 Hz tone for a '0'.
All in all extremely slow by modern standards.

Since I didn't have a cassette recorder or tapes anymore, I went out on the internet to get some used stuff.
I found a used Philips cassette recorder for € 15 still in good shape and 10 brand new tapes that were still shrink-wrapped.
